...
pub fn get_best_lengths(s: &mut ZopfliBlockState, in_data: &[u8], instart: size_t, inend: size_t, costmodel: fn (c_uint, c_uint, Option<&SymbolStats>) -> c_double, costcontext: Option<&SymbolStats>) -> (c_double, Vec<c_ushort>) {
    ...
    let mut costs = vec![f32::MAX; blocksize + 1];
...
pub fn lz77_optimal_run(s: &mut ZopfliBlockState, in_data: &[u8], instart: size_t, inend: size_t, costmodel: fn (c_uint, c_uint, Option<&SymbolStats>) -> c_double, costcontext: Option<&SymbolStats>, store: &mut Lz77Store) {
...
pub fn lz77_optimal_fixed(s: &mut ZopfliBlockState, in_data: &[u8], instart: size_t, inend: size_t, store: &mut Lz77Store) {
    ...
    s.blockend = inend;
    lz77_optimal_run(s, in_data, instart, inend, GetCostFixed, None, store);
}
