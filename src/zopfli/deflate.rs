...
use symbols::{ZopfliGetLengthSymbol, ZopfliGetDistSymbol, ZopfliGetLengthSymbolExtraBits, ZopfliGetDistSymbolExtraBits, ZopfliGetLengthExtraBitsValue, ZopfliGetLengthExtraBits, ZopfliGetDistExtraBitsValue, ZopfliGetDistExtraBits};
...
pub extern fn AddLZ77Block(options_ptr: *const ZopfliOptions, btype: c_int, final_block: c_int, in_data: *const c_uchar, lz77: *mut ZopfliLZ77Store, lstart: size_t, lend: size_t, expected_data_size: size_t, bp: *const c_uchar, out: *const *const c_uchar, outsize: *const size_t) {
    ...
    for i in lstart..lend {
        uncompressed_size += if unsafe { *(&*lz77).dists.offset(i as isize) } == 0 {
            ...
            unsafe {
                *(&*lz77).litlens.offset(i as isize) as size_t
            }
        };
    }
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern fn AddLZ77Data(lz77: *const ZopfliLZ77Store, lstart: size_t, lend: size_t, expected_data_size: size_t , ll_symbols: *const c_uint, ll_lengths: *const c_uint, d_symbols: *const c_uint, d_lengths: *const c_uint, bp: *const c_uchar, out: *const *const c_uchar, outsize: *const size_t) {
    let mut testlength: size_t = 0;

    for i in lstart..lend {
        let dist: c_uint = unsafe { *(&*lz77).dists.offset(i as isize) } as c_uint;
