...
use hash::ZopfliHash;
use util::{ZopfliGetLengthSymbol, ZopfliGetDistSymbol, ZOPFLI_NUM_LL, ZOPFLI_NUM_D, ZOPFLI_MAX_MATCH, ZOPFLI_MIN_MATCH, ZOPFLI_WINDOW_MASK, ZOPFLI_MAX_CHAIN_HITS, ZOPFLI_WINDOW_SIZE};
...
#[no_mangle]
#[allow(non_snake_case)]
pub extern fn ZopfliLZ77Greedy(s_ptr: *mut ZopfliBlockState, in_data: *mut c_uchar, instart: size_t, inend: size_t, store_ptr: *mut ZopfliLZ77Store, _hash_ptr: *mut ZopfliHash) {
    let store = unsafe {
        assert!(!store_ptr.is_null());
        &mut *store_ptr
    };
